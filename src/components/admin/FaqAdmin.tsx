...
import { createFaqQuestion, deleteFaqQuestion, getFaqQuestions } from "@/lib/actions/faq";
...
export default function FaqAdmin({ initialQuestions }: { initialQuestions: FaqQuestion[] }) {
  ...
  async function handleCreate(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await createFaqQuestion(formData);
      if (result?.error) {
  ...
  return (
    <div>
      ...
      {/* Create Modal */}
      <Modal title="Yeni Soru Ekle" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} size="lg">
        <form action={handleCreate} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            ...
            <input id="faq-q-text" name="question_text" type="text" className="form-input" placeholder="Soruyu yazın..." required />
          </div>
          <div>
            <label className="form-label" htmlFor="faq-order">Sıra (display_order)</label>
            <input id="faq-order" name="display_order" type="number" className="form-input" defaultValue={questions.length} min={0} />
          </div>
          ...
          <div>
            ...
            <select id="correct_option" name="correct_option" className="form-input" required>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
            ...
            <button type="submit" className="btn-primary" disabled={isPending}>
              {isPending ? "Kaydediliyor..." : "Soruyu Ekle"}
            </button>
          </div>
        </form>
      </Modal>
